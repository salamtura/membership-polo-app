<x-app-layout>
    ...
    @php
        ...
        $notice_board_items = [
            [
                'datelabel' => 'Today',
                'day' => '18',
                'title' => 'Ministerial Cup Finals',
                'description' =>
                    'A polo event is a thrilling display of skill, strategy, and horsemanship, where teams of players on horseback compete to score goals by driving a small ball into the opposing team’s net using a long-handled mallet.',
                'players' => 7,
                'breakoutSessions' => 4,
                'time' => '10:00 AM',
            ],
    ...
    <div class="flex justify-center p-5 relative sm:mx-10">
        <div class="flex flex-col gap-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="common-card p-[20px] gap-[30px]" style="flex: 1; border: 1px solid #ddd;">
                        ...
                        <div class="w-full md:w-3/12 md:mx-2">
                            ...
                            <div class="bg-white p-3 border-t-4 border-green-400">
                                ...
                                <h3 class="text-gray-600 font-lg text-semibold leading-6">
                                    {{ $user->membership->name_of_organization }}</h3>
                                ...
                                <ul
                                    ...
                                    <li class="flex items-center py-3">
                                        ...
                                        <span
                                            class="ml-auto">{{ date('Y', strtotime($user->membership->membership_since)) }}</span>
                                    </li>
                                </ul>
                            </div>
                            <!-- End of profile card -->
                        ...
                        <div class="w-full md:w-9/12 mx-2 ">
                            ...
                            @else
                                <div class=" grid md:grid-cols-2 sm:grid-cols-1">
                                    ...
                                    @endif
                                    <div
                                        class="w-full max-w-md p-4 {{ $pitch->status == 'open' ? 'bg-green-500' : 'bg-red-500' }} border border-gray-200 rounded-lg shadow sm:p-8">
                                        <h5 class="text-xl font-medium text-black">Pitch Status</h5>
                                        <div
                                            class="flex items-baseline {{ $pitch->status == 'open' ? 'text-green-900' : 'text-red-900' }}">
                                            <span
                                                class="text-5xl font-extrabold tracking-tight">{{ \Illuminate\Support\Str::upper($pitch->status) }}</span>
                                            {{--                                <span class="ml-1 text-xl font-normal text-gray-500 dark:text-gray-400">/month</span> --}}
                                        </div>
                                        <ul role="list" class="space-y-5 my-7">
                                            <li class="flex space-x-3 items-center">
                                                <svg class="flex-shrink-0 w-4 h-4 text-black" aria-hidden="true"
                                                ...
                                                <span
                                                    class="text-base font-normal leading-tight text-white">{{ \Illuminate\Support\Str::upper($pitch->description) }}</span>
                                            </li>
                                            @if ($pitch->status == 'open')
                                                <li class="flex space-x-3 items-center">
                                                    ...
                                                    <span
                                                        class="text-base font-normal leading-tight text-white">{{ 'From ' . date('l, M d - H:ia', strtotime($pitch->from_date)) . ' to ' . date('l, M d H:ia', strtotime($pitch->to_date)) }}</span>
                                                </li>
                                            @endif
                                        </ul>
                                    </div>
                                    ...
                                    @endif
                                    <div
                                        class="w-full max-w-md p-4 {{ $chukker->status == 'open' ? 'bg-green-500' : 'bg-red-500' }} border border-gray-200 rounded-lg shadow sm:p-8 ">
                                        <h5 class="text-xl font-medium text-black">Chukker Booking</h5>
                                        <div
                                            class="flex items-baseline {{ $chukker->status == 'open' ? 'text-green-900' : 'text-red-900' }}">
                                            <span
                                                class="text-5xl font-extrabold tracking-tight">{{ \Illuminate\Support\Str::upper($chukker->status) }}</span>
                                        ...
                                        <ul role="list" class="space-y-5 my-7">
                                            <li class="flex space-x-3 items-center">
                                                ...
                                                <span
                                                    class="text-base font-normal leading-tight text-white dark:text-gray-400">{{ \Illuminate\Support\Str::upper($chukker->chukker_no) }}</span>
                                            </li>
                                            <li class="flex space-x-3 items-center">
                                                <svg class="flex-shrink-0 w-4 h-4 text-black" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                                ...
                                                <span
                                                    class="text-base font-normal leading-tight text-white dark:text-gray-400">{{ Carbon\Carbon::parse($chukker->chukker_date)->format('l d F Y') }}</span>
                                            </li>
                                        </ul>
                                        @if ($chukker->status == 'open')
                                            @if ($booking == null)
                                                <button type="button" onclick="modalHandler(true)"
                                                    class="text-white w-full bg-green-900 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-blue-200 font-large rounded-lg text-lg mt-5 px-5 py-2 justify-center  text-center">Book
                                                    Now</button>
                                            @else
                                ...
                                <div class="grid md:grid-cols-4 sm:grid-cols-2">
                                    <div class="bg-white m-3 p-3 shadow-sm rounded-sm text-center">
                                        ...
                                        <div class="text-center">
                                            <div class="text-teal-600 text-xl font-bold">{{ $invoices->count() }}
                                            </div>
                                            <div class="text-gray-500 text-xs">Unpaid</div>
                                            <div class="text-green-500 text-xs"><a
                                    ...
                                    <div class="bg-white m-3 p-3 shadow-sm rounded-sm text-center">
                                        ...
                                        <div class="text-center">
                                            <div class="text-teal-600 text-xl font-bold">
                                                {{ $user->membership->subscriptions->count() }}</div>
                                            <div class="text-gray-500 text-xs">Unpaid</div>
                                            <div class="text-green-500 text-xs"><a
                                                    href="{{ route('subscriptions') }}">view
                                                    more</a></div>
                                        </div>
                                    </div>
                                    <div class="bg-white m-3 p-3 shadow-sm rounded-sm text-center">
                                        ...
                                        <div class="text-center">
                                            <div class="text-teal-600 text-xl font-bold">{{ $penalties->count() }}
                                            </div>
                                            <div class="text-gray-500 text-xs">Pending</div>
                                            <div class="text-green-500 text-xs"><a
                                                    href="{{ route('penalties') }}">view
                                                    more</a></div>
                                        </div>
                                    </div>
                                    <div class="bg-white m-3 p-3 shadow-sm rounded-sm text-center">
                                            ...
                                            @foreach ($notices as $notice)
                                                <li>
                                                    <div class="text-teal-600">
                                                        <a
                                                            href="/post-details/{{ $notice->id }}">{{ $notice->title }}</a>
                                                    </div>
                                                    <div class="text-gray-500 text-xs">{{ $notice->created_at }}
                                                    </div>
                                                </li>
                                            @endforeach
                                        </ul>
                                            ...
                                            @foreach ($user->membership->stables as $stable)
                                                <ul class="list-inside space-y-2 pl-3">
                                                    <li class="grid grid-cols-2">
                                                        ...
                                                        <div class="text-gray-500">{{ $stable->description }}
                                                        </div>
                                                    </li>
                                                    <li class="grid grid-cols-2">
                                                        <div class="text-teal-600">Number of Boxes</div>
                                                        <div class="text-gray-500 ">{{ $stable->number_of_boxes }}
                                                        </div>
                                                    </li>
                                                    <li class="grid grid-cols-2">
                                                        <div class="text-teal-600">Number of Rooms</div>
                                                        <div class="text-gray-500 ">{{ $stable->number_of_rooms }}
                                                        </div>
                                                    </li>
                                                    <li class="grid grid-cols-2">
                                                        <div class="text-teal-600 grid-flow-dense">Number of
                                                            Lounges</div>
                                                        <div class="text-gray-500 ">
                                                            {{ $stable->number_of_lounges }}</div>
                                                    </li>
                                                    <li class="grid grid-cols-2">
                                                        <div class="text-teal-600">Number of Stores</div>
                                                        <div class="text-gray-500 ">
                                                            {{ $stable->number_of_stores }}</div>
                                                    </li>
                                                </ul>
                                            @endforeach
                                        @else
                                ...
                                <div class="text-gray-700">
                                    <div class="grid md:grid-cols-2 text-sm">
                                        ...
                                        <div class="grid grid-cols-2">
                                            ...
                                            <div class="px-4 py-2">
                                                {{ date('d F', strtotime($user->membership->date_of_birth)) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            @foreach ($members as $member)
                                                <div class="text-center my-2">
                                                    <img class="h-16 w-16 rounded-full mx-auto"
                                                        ...
                                                        alt="">
                                                    <a href="#"
                                                        class="text-main-color">{{ $member->user->name }}</a>
                                                </div>
                                            @endforeach
                                        </div>
                                        <a href="/members"
